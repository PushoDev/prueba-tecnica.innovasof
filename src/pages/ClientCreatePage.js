  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        sexo: formData.sexo,
        resennaPersonal: formData.resennaPersonal || "", // Avoid null if possible
      ...
      showNotification('¡Éxito! Cliente creado correctamente');

      setTimeout(() => navigate('/clientes'), 1500);
    } catch (err) {
      console.error('--- ERROR CREACIÓN ---');
      if (err.response) {
        console.error('Status:', err.response.status);
        console.error('Data:', JSON.stringify(err.response.data, null, 2));
      }
      const errorMsg = err.response?.data?.message || err.message || 'Error al crear el cliente';
      ...
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2 }}>
        ...
        <form onSubmit={handleSubmit}>
          <Grid container spacing={2}>
            {/* Grid size follows user preference: flat size prop */}
